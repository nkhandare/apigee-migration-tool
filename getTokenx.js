    ...
    let parsed = url.parse(req.url),
        ...
        request = https.request(options, (res) => {
          ...
          res.on('end', () => {
            ...
            // resolve(JSON.parse(payload).token_type);
          });
...
function getGoogleAuthJwt(keyfile) {
  console.log(keyfile);
...
function redeemJwtForAccessToken(ctx) {
  ...
  return httpRequest({verbose:true, req});
}
...
function main(args) {
  try {
    ...
    if (apigee.to.keyfile) {
      keyfile = JSON.parse(fs.readFileSync(apigee.to.keyfile, 'utf8'));
      getGoogleAuthJwt({keyfile})
